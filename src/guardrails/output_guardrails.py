...
async def llm_safety_check(response_text: str) -> dict:
    ...
    try:
        ...
        print(f"Warning: Safety judge error: {e}")
        # Default to safe if judge fails (fail-open) or unsafe (fail-closed)
        # In banking, fail-closed is safer, but for lab let's fail-open with a warning
        ...
        *,
        ...
        if self.use_llm_judge:
            ...
            if not judge_result["safe"]:
                ...
                llm_response.content = types.Content(
                    role="model",
                    parts=[types.Part.from_text(text="I'm sorry, but I cannot provide that information as it may violate my safety guidelines.")],
                )
